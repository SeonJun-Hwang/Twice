...
public class ETIITIIViewHolder extends BaseViewHolder {

    private ViewPager contentImageList;
    private TextView imageCountText;
    private ViewPager extContentImageList;
    private TextView extImageCountText;

    public ETIITIIViewHolder(View itemView) {
        super(itemView);

        contentImageList = (ViewPager) itemView.findViewById(R.id.card_content_image_view);
        extContentImageList = (ViewPager) itemView.findViewById(R.id.card_content_ext_image_view);
        imageCountText = (TextView) itemView.findViewById(R.id.card_content_image_count_text);
        extImageCountText = (TextView) itemView.findViewById(R.id.card_content_ext_text);
    }

    @Override
    protected void setImageContent(Post post) {
        ArrayList<PostMedia> imageList = post.getImageList();
        ArrayList<PostMedia> extImageList = post.getExtendPost().getImageList();
...
//            Log.e(TAG, "Post " + i + " has " + imageList.size() + " Images");
    ...
    protected void setExtendField(Post extPost) {
        ArrayList<PostMedia> imageList = extPost.getExtendPost().getImageList();
